...
const ButtonsProductComponent = ({
  ...
  const handleBuyWhatsApp = useCallback(() => {
    if (onBuyWhatsApp) {
      ...
      const message = encodeURIComponent(`${t('product.whatsappPrefix')} ${productTitle}`);

      window.open(`https://wa.me/${whatsappNumber}?text=${message}`, "_blank");
    }
  }, [onBuyWhatsApp, whatsappNumber, productTitle, t]);
  ...
  return (
    <div className="flex flex-col items-start gap-2 w-full">
      ...
      >
        <div className="flex flex-row justify-center items-center px-4 py-2.5 gap-2 bg-[#B6D6DD] rounded-full w-full transition-all hover:opacity-80">
